...
export const action = async ({ request }) => {
  ...
  try {
    ...
    if (!shop) {
      return corsJsonResponse(formatError("Missing required field: 'shop'", 400), request, { status: 400 });
    }
    if (!prices || !Array.isArray(prices) || prices.length === 0) {
      return corsJsonResponse(formatError("Missing or invalid 'prices' array.", 400), request, { status: 400 });
    }
    ...
    if (!draftOrderResult.success) {
      return corsJsonResponse(formatError("Failed to create comparison draft order", 400, draftOrderResult.userErrors), request, { status: 400 });
    }
    ...
    return corsJsonResponse({
      ...
      shop,
      comparison: {
        expectedUiTotal: expectedUiTotal ? parseFloat(expectedUiTotal) : null,
        backendCalculatedTotal: backendPricing.finalTotal,
        shopifyReturnedTotal: draftOrderResult.total,
        matchesBackendAndShopify: backendPricing.finalTotal === draftOrderResult.total,
        matchesUiAndBackend: expectedUiTotal ? parseFloat(expectedUiTotal) === backendPricing.finalTotal : null,
      },
      draftOrderDetails: {
